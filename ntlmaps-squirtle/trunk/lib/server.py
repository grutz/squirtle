...
class AuthProxyServer:
    ...
    def __init__(self, config):
        ...
        if len(self.config['NTLM_AUTH']['PASSWORD']) == 65 and self.config['NTLM_AUTH']['PASSWORD'][32] == ':':
            # pass the hash
            self.config['NTLM_AUTH']['LM_HASHED_PW'] == self.config['NTLM_AUTH']['PASSWORD'][:32]
            self.config['NTLM_AUTH']['LM_HASHED_PW'] == self.config['NTLM_AUTH']['PASSWORD'][33:]
        else:
